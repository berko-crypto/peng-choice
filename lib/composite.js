...
const { fetchImageBuffer } = require('./pudgyImages');
...
async function fetchImage(tokenId) {
  const buf = await fetchImageBuffer(tokenId);
...
async function buildMatchupImage(tokenIdA, tokenIdB) {
  const [imgA, imgB] = await Promise.all([fetchImage(tokenIdA), fetchImage(tokenIdB)]);
